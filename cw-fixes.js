...
(function(){
  ...
  function setupSearchAutocomplete(){
    ...
    // Reuse or create the body-level dropdown
    let drop=document.getElementById('fw-ac');
    if(!drop){
      drop=document.createElement('div');
      drop.id='fw-ac';
      drop.style.cssText='position:fixed;z-index:9999;background:#1E293B;border:1px solid rgba(56,189,248,.3);border-radius:0 0 14px 14px;overflow:hidden;display:none;box-shadow:0 12px 32px rgba(0,0,0,.55);max-height:220px;overflow-y:auto';
    ...
    input.addEventListener('input',()=>{
      ...
      debTimer=setTimeout(async()=>{
        try{
          const r=await fetch(`https://nominatim.openstreetmap.org/search?format=json&limit=5&addressdetails=0&q=${encodeURIComponent(q)}`,
            {headers:{'Accept-Language':'en'}});
          const items=await r.json();
          if(!items.length){drop.style.display='none';return;}
          drop.innerHTML=items.map(it=>{
            const label=it.display_name.split(',').slice(0,3).join(', ');
            const t=it.type||it.class||'';
            const icon=t.includes('city')||t.includes('town')?'🏙️':t.includes('village')?'🏘️':t.includes('road')||t.includes('street')?'🛣️':'📍';
            const safe=label.replace(/"/g,'&quot;').replace(/'/g,'&#39;');
            return`<div onmousedown="event.preventDefault()"
              onclick="document.getElementById('fw-loc').value='${safe}';document.getElementById('fw-ac').style.display='none'"
              style="padding:11px 14px;font-size:13px;color:#EFF6FF;cursor:pointer;border-bottom:1px solid rgba(255,255,255,.06);display:flex;gap:9px;align-items:center"
              onmouseover="this.style.background='rgba(56,189,248,.12)'" onmouseout="this.style.background=''">
    ...
    input.addEventListener('focus',positionDrop);
    input.addEventListener('keydown',e=>{
      if(e.key==='Escape'||e.key==='Enter')drop.style.display='none';
    });
    document.addEventListener('scroll',positionDrop,true);
