...
(function(){
  ...
  window.cwReloadRoute=function(i){
    ...
    // Compatibility: older builds accidentally stored the raw GPX XML in ptsKey
    // instead of saving it as gpxKey. Recover it without opening the file picker.
    if(typeof r.ptsKey==='string'&&/<gpx\b/i.test(r.ptsKey)&&window.cwLoadGPXContent){
      const content=r.ptsKey;
      try{
        const key='cw_gpx_'+Date.now();
        localStorage.setItem(key,content);
        r.gpxKey=key;
        r.ptsKey=null;
        saveRoutes(routes);
      }catch(e){}
      window.cwLoadGPXContent(content,r.name);
      return;
    }
    ...
    if(r.ptsKey){
      try{
        const raw=localStorage.getItem(r.ptsKey);
        const pts=raw?JSON.parse(raw):null;
    ...
    // Fallback: ask user to re-upload only when no usable stored route exists
