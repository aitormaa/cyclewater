...
(function(){
  ...
  // ─── FINDWATER CACHE (24h) ─────────────────────────────────────
  function fwKey(loc,r){return`cw_fw_v2_${(loc||'').toLowerCase().trim()}_${r}`;}
  ...
  function patchQOverpass(){
    ...
    window._cwBaseQOverpass=window.qOverpass;
    window.qOverpass=async function(rpts,buf){
      const f=rpts[0],l=rpts[rpts.length-1],n=rpts.length;
      const ckey=`cw_rt_v3_${f.lat.toFixed(3)}_${f.lng.toFixed(3)}_${l.lat.toFixed(3)}_${l.lng.toFixed(3)}_${n}_${buf}`;
      ...
      const q=`[out:json][timeout:15];(node["amenity"="drinking_water"](${bb});node["man_made"="water_tap"]["drinking_water"!="no"](${bb});node["natural"="spring"]["drinking_water"="yes"](${bb});node["amenity"="fountain"](${bb});node["amenity"="toilets"]["drinking_water"="yes"](${bb}););out tags;`;
      let d;try{d=await fastOverpass(q);}catch(e){d=null;}
      let elements=Array.isArray(d?.elements)?d.elements:[];
      // If the fast bbox response is empty or malformed, fall back to the original corridor query.
      if(!elements.length&&window._cwBaseQOverpass){try{const fb=await window._cwBaseQOverpass(rpts,buf);if(Array.isArray(fb)&&fb.length){try{localStorage.setItem(ckey,JSON.stringify({pts:fb,ts:Date.now()}));}catch(e){}return fb;}}catch(e){}}
      if(!elements.length){if(d)return [];throw new Error('No waterpoint response');}
      const seen=new Set();
      const raw=elements.filter(e=>{if(seen.has(e.id))return false;seen.add(e.id);return true;}).map(e=>{const lat=e.center?e.center.lat:e.lat,lng=e.center?e.center.lon:e.lon;return{id:`osm_${e.id}`,lat,lng,water_type:e.tags?.amenity||e.tags?.man_made||e.tags?.natural||e.tags?.landuse||'drinking_water',name:e.tags?.name||null,seasonal:e.tags?.seasonal==='yes',source:'osm'};});
      // Post-filter: only keep points within buf metres of the route
      const pts=raw.filter(p=>ptNearRoute(p,rpts,buf));
      if(pts.length)try{localStorage.setItem(ckey,JSON.stringify({pts,ts:Date.now()}));}catch(e){}
      return pts;
    };
  }
  ...
  function refreshMyPtsLayer(){
    const map=getMap();if(!map){if(window.S)window.S._waterTabAdding=false;return;}
    const pts=getMyPts();
...
<div class="card" style="margin-top:10px">
  ...
  function applyTabLayers(){
    ...
    if(v==='water'){
      ...
      if(S){
        const act=(S.waterPts||[]).filter(w=>!REPORTED.has(w.id)&&!window.cwIsHidden?.(w));
        ...
        const nearComm=rpts.length
          ? (S.commPts||[]).filter(w=>!window.cwIsHidden?.(w)&&ptNearRoute(w,rpts,1000))
          : [];
        map.getSource('wcomm')?.setData(fc(nearComm));
      }
    }
  }
  ...
  function patchRefreshLayers(){
    ...
    window.refreshLayers=function(){
      ...
      const REPORTED=window.REPORTED||new Set();
      let act=S.waterPts.filter(w=>!REPORTED.has(w.id)&&!window.cwIsHidden?.(w));
      ...
      map.getSource('wosm')?.setData(fc(act));
      map.getSource('wrep')?.setData(fc(rep));
      map.getSource('wcomm')?.setData(fc((S.commPts||[]).filter(w=>!window.cwIsHidden?.(w))));
      map.getSource('wfw')?.setData(fc(S.fwPts||[]));
  ...
  window.cwFindWater=async function(){
    ...
    if(cached){
      ...
      const rm=r*1000;
      const q=`[out:json][timeout:25];(node["amenity"="drinking_water"](around:${rm},${lat},${lng});node["amenity"="fountain"](around:${rm},${lat},${lng});node["natural"="spring"]["drinking_water"="yes"](around:${rm},${lat},${lng});node["man_made"="water_tap"]["drinking_water"!="no"](around:${rm},${lat},${lng}););out;`;
      try{
  ...
  window.cwWaterTabAdd=function(){
    ...
    if(!form){
      ...
      form.style.cssText='position:fixed;bottom:calc(76px + env(safe-area-inset-bottom));left:12px;right:12px;z-index:2000;background:rgba(13,20,38,.96);border:1px solid rgba(255,255,255,.12);border-radius:18px;padding:16px;backdrop-filter:blur(14px);box-shadow:0 8px 32px rgba(0,0,0,.5)';
...
<div style="display:flex;gap:8px">
  ...
  <button id="wt-save-btn" onclick="cwWaterTabSave()" style="flex:2;padding:10px;border:none;border-radius:10px;background:linear-gradient(135deg,#4ADE80,#22C55E);color:#fff;font-size:13px;font-weight:700;cursor:pointer" disabled>Save</button>
    ...
    if(form.parentNode){form._lat=null;form._lng=null;const s=document.getElementById('wt-save-btn');if(s){s.disabled=true;s.textContent='Save';}}
  };
  ...
  window.cwWaterTabTapMap=function(){
    ...
    const handler=e=>{
      map.getCanvas().style.cursor='';map.off('click',handler);if(window.S)window.S._waterTabAdding=false;
  ...
  window.cwWaterTabSave=async function(){
    ...
    const btn=document.getElementById('wt-save-btn');if(btn){btn.disabled=true;btn.textContent='Saving…';}
    const ident=getIdent()||{};
    const ok=await window.saveCommPt?.({lat:f._lat,lng:f._lng,water_type:type,notes:note,contributor_name:ident.name||window.getNickname?.()||'Anonymous',contributor_email:ident.email||null,points:10});
    if(!ok){if(btn){btn.disabled=false;btn.textContent='Save';}return;}
    const tot=parseInt(localStorage.getItem('cw_total_wpts')||'0')+1;localStorage.setItem('cw_total_wpts',String(tot));
    window.refreshLayers?.();f.remove();window.cwWQAddedWater?.();window.showToast?.('✅ Water point saved');
  };

  function injectWaterTabAddButton(){
    const card=document.getElementById('wo-card');if(!card||document.getElementById('wt-add-open'))return;
    const b=document.createElement('button');b.id='wt-add-open';b.textContent='➕ Add water point';
    b.onclick=()=>window.cwWaterTabAdd?.();
    b.style.cssText='width:100%;margin-top:10px;padding:10px;border:1px solid rgba(56,189,248,.35);border-radius:11px;background:rgba(56,189,248,.12);color:#38BDF8;font-size:13px;font-weight:700;cursor:pointer';
    card.appendChild(b);
  }
  setTimeout(injectWaterTabAddButton,0);
  ...
  function init(){
    ...
    patchResetRoute();
    patchRouteBanner();
    // Keep the original corridor query as the stable default.
    // patchQOverpass();
    patchUpdateStats();
    patchSubmitPt();
    patchWaterTabSave();
