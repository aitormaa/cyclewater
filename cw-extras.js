...
(function(){
  ...
  window.importKomoot=async function(){
    ...
    async function tryFetch(targetUrl){
      for(const proxy of PROXIES){
        try{
          ...
          if(r.ok)return r;
        }catch(e){if(e.name==='AbortError')throw e;}// if aborted, stop trying
      }
    ...
    try{
      ...
      // Komoot stores route points in _embedded.coordinates.items
      ...
      window.cwSaveRoute?.(name,pts.length); // save to recent routes if available
      ...
      clearTimeout(timer);
      if(e.name==='AbortError'){showToast('⚠️ Timed out — try downloading the GPX from Komoot and uploading it directly');}
      else if(e.message?.includes('403')||e.message?.includes('401')){showToast('⚠️ Komoot blocked access — download the GPX file from Komoot and upload it here');}
      else{showToast('⚠️ Could not load tour — try uploading the GPX file directly from Komoot');}
    }finally{
...
<div class="card" style="margin-top:10px">
  ...
  function applyTabLayers(){
    ...
    if(v==='profile'){
      ...
      ['route-l','route-h'].forEach(id=>{try{map.setLayoutProperty(id,'visibility','none');}catch(e){}});
