...
(function(){
  ...
  window.importKomoot=async function(){
    ...
    try{
      // Komoot blocks direct browser fetches (CORS) — use a proxy for public tours
      const apiUrl=`https://www.komoot.com/api/v007/tours/${tourId}?hl=en`;
      const r=await fetch(`https://corsproxy.io/?${encodeURIComponent(apiUrl)}`,{signal:controller.signal});
      clearTimeout(timer);
      ...
      clearTimeout(timer);
      if(e.name==='AbortError'){showToast('⚠️ Timed out — is the tour public on Komoot?');}
      else if(e.message?.includes('403')){showToast('⚠️ Tour is private — set it to Public on Komoot first');}
      else{showToast('⚠️ Could not load tour — check the URL and try again');}
    }finally{
