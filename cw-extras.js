...
<div class="card" style="background:linear-gradient(135deg,rgba(14,165,233,.1),rgba(56,189,248,.05));border-color:rgba(14,165,233,.2)">
  ...
  // ─── SWIPE — 3 panel states: closed / half / open ────────────
  function initSwipe(){
    ...
    hdr.addEventListener('touchend',e=>{
      ...
      if(Math.abs(dy)<20)return; // ignore tiny movements
      if(dy>0){ // swipe down
  ...
  window.cwFindWater=async function(){
    ...
    if(map){
      ...
      map.flyTo({center:[lng,lat],zoom:r<=2?15:r<=5?14:r<=15?12:10,speed:1.4});
    }
    if(res){
