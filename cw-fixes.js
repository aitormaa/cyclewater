...
(function(){
  ...
  // ─── CLUSTER LAYERS (#5) — add after wosm source is ready ─────
  ...
  // Patch updateStats — also fix time overflow (3h 60m → 4h 0m) + first-water + breakdown
  const _origUS=window.updateStats;
  window.updateStats=function(){
    _origUS?.();
    // Fix time: m>=60 should carry into hours
    ['so-time'].forEach(id=>{
      const el=document.getElementById(id);if(!el)return;
      const mt=el.textContent.match(/^(\d+)h (\d+)m$/);
      if(mt){
  ...
  // Fix Live Mode — better GPS error feedback
  const _origTL=window.toggleLive;
  window.toggleLive=function(){
    const wasOff=!window._liveOn;
    _origTL?.();
    if(wasOff&&window._liveOn===undefined){
      // fallback: check button state
    }
    if(wasOff){
      const map=window.map||window.cwMap;
      if(navigator.geolocation&&map){
        // replace the silent error handler
        if(window.watchId!==null&&window.watchId!==undefined){
          ...
          window.watchId=navigator.geolocation.watchPosition(
            ...
            (err)=>{
              ...
              window.showToast?.('⚠️ '+( msgs[err.code]||'GPS error'));
              // Turn off live mode on persistent errors
              window.toggleLive?.();
            },
  ...
  // ─── 4. SAFETY NET — force route visible after every data refresh ─
  // Wraps the already-patched refreshLayers to guarantee route-l/route-h
  // are never hidden when the user is on panel or split tab
