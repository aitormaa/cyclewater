...
(function(){
  ...
  function patchQOverpass(){
    ...
    window.qOverpass=async function(rpts,buf){
      ...
      const q=`[out:json][timeout:20];(node["amenity"="drinking_water"](${bb});node["man_made"="water_tap"]["drinking_water"!="no"](${bb});node["natural"="spring"]["drinking_water"="yes"](${bb});node["amenity"="fountain"]["drinking_water"="yes"](${bb});node["amenity"="grave_yard"](${bb});node["landuse"="cemetery"](${bb}););out;`;
  ...
  // ─── KOMOOT DIRECT IMPORT ────────────────────────────────────
  function injectKomootImport(){
    const uz=document.getElementById('upload-zone');
    if(!uz||document.getElementById('komoot-row'))return;
    const row=document.createElement('div');
    row.id='komoot-row';
    row.style.cssText='display:flex;gap:8px;margin-top:8px;align-items:center';
    row.innerHTML=`
      <div style="display:flex;align-items:center;gap:6px;flex:1;background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.1);border-radius:12px;padding:0 12px;height:40px">
        <span style="font-size:15px">🚵</span>
        <input id="komoot-url" type="url" inputmode="url" placeholder="Paste route URL (Komoot, Strava, direct .gpx link…)" autocorrect="off" spellcheck="false"
          style="flex:1;background:none;border:none;color:#EFF6FF;font-size:12px;outline:none;min-width:0;caret-color:#38BDF8"
          onkeydown="if(event.key==='Enter')window.importKomoot()">
      </div>
      <button onclick="window.importKomoot()" id="komoot-btn"
        style="background:linear-gradient(135deg,#0EA5E9,#38BDF8);border:none;border-radius:12px;color:#fff;font-size:12px;font-weight:600;padding:0 14px;height:40px;cursor:pointer;white-space:nowrap;flex-shrink:0">
        Import
      </button>`;
    uz.after(row);
  }
  ...
  window.importKomoot=async function(){
    ...
    if(!url){showToast('⚠️ Paste a Komoot tour URL first');return;}

    // Extract numeric tour ID from URL  e.g. komoot.com/tour/1234567890
    const m=url.match(/komoot\.com\/(?:tour|highlight)\/(\d+)/i);
    if(!m){showToast('⚠️ Invalid URL — needs to be a komoot.com/tour/… link');return;}
    const tourId=m[1];
    ...
    // Multiple CORS proxies to try in order (Komoot blocks some)
    const PROXIES=[
    ...
    async function tryFetch(targetUrl){
      for(const proxy of PROXIES){
        try{
          const r=await fetch(proxy(targetUrl),{signal:controller.signal,headers:{'Accept':'application/json'}});
          if(r.ok)return r;
    ...
    try{
      ...
      if(items.length<2) throw new Error('Empty route');

      const pts=items.map(p=>({lat:p.lat,lng:p.lng,ele:p.alt||0}));
      const name=data.name||`Komoot ${tourId}`;

      window.S.routeName=name;
      window.S.routePts=pts;
      const uz=document.getElementById('upload-zone');
      if(uz&&window.routeBanner){uz.innerHTML=window.routeBanner(name);uz.onclick=null;}
      // Save pts to localStorage so recent-routes can reload without re-upload
      const _pk='cw_kmt_'+tourId;
      try{localStorage.setItem(_pk,JSON.stringify(pts));}catch(e){}
      let _km=0;for(let i=1;i<pts.length;i++){const p=pts[i-1],c=pts[i],dA=(c.lat-p.lat)*Math.PI/180,dO=(c.lng-p.lng)*Math.PI/180,a=Math.sin(dA/2)**2+Math.cos(p.lat*Math.PI/180)*Math.cos(c.lat*Math.PI/180)*Math.sin(dO/2)**2;_km+=2*6371*Math.asin(Math.sqrt(a));}
      window.cwSaveRoute?.(name,_km,null,_pk);
      ...
      clearTimeout(timer);
      // Komoot API requires login even for public tours — guide user to download GPX instead
      window.showKomootGpxHelper?.(tourId);
    }finally{
      if(btn){btn.textContent='Import';btn.disabled=false;}
    }
  };

  // ─── MY WATER POINTS (personal store) ───────────────────────
  function getMyPts(){try{return JSON.parse(localStorage.getItem('cw_my_pts')||'[]');}catch(e){return[];}}

  function addMyPt(pt){
    if(!pt.lat||!pt.lng)return;
    const pts=getMyPts();
    if(!pts.find(p=>p.id===pt.id))pts.push(pt);
    try{localStorage.setItem('cw_my_pts',JSON.stringify(pts.slice(-500)));}catch(e){}
    refreshMyPtsLayer();
    updateMyPtsList();
    // update water pts counter
    const tot=Math.max(parseInt(localStorage.getItem('cw_total_wpts')||'0'),pts.length);
    localStorage.setItem('cw_total_wpts',String(tot));
  }

  window.cwDeleteMyPt=function(id){
    const pts=getMyPts().filter(p=>p.id!==id);
    try{localStorage.setItem('cw_my_pts',JSON.stringify(pts));}catch(e){}
    refreshMyPtsLayer();updateMyPtsList();
    showToast('🗑 Point removed');
  };

  function refreshMyPtsLayer(){
    ...
    try{map.getSource('wmine')?.setData(fc);}catch(e){}
    const badge=document.getElementById('my-pts-count');if(badge)badge.textContent=pts.length||'';
  }

  function updateMyPtsList(){
    const list=document.getElementById('my-pts-list');if(!list)return;
    const pts=getMyPts();
    if(!pts.length){list.innerHTML='<p style="color:var(--tm);font-size:12px;text-align:center;padding:16px 0">No water points added yet<br><span style="font-size:10px;opacity:.7">Add via the Water or Panel tab</span></p>';return;}
    const icon=t=>t==='fountain'?'⛲':t==='tap'?'🚰':t==='spring'?'💦':t==='shop'?'🏪':t==='bar'?'☕':'💧';
    list.innerHTML=pts.slice().reverse().map(p=>`
      <div style="display:flex;align-items:center;gap:8px;padding:9px 0;border-bottom:1px solid rgba(255,255,255,.06)">
        <span style="font-size:18px;flex-shrink:0">${icon(p.water_type)}</span>
        <div style="flex:1;min-width:0">
          <div style="font-size:12px;color:#EFF6FF;font-weight:600;text-transform:capitalize">${(p.water_type||'Water point').replace(/_/g,' ')}</div>
          <div style="font-size:10px;color:#7B91B0;margin-top:1px">${p.lat?.toFixed(4)}, ${p.lng?.toFixed(4)}${p.notes?' · '+p.notes.slice(0,30):''}</div>
        </div>
        <button onclick="cwDeleteMyPt('${p.id}')" style="background:rgba(248,113,113,.1);border:1px solid rgba(248,113,113,.2);color:#F87171;border-radius:6px;padding:4px 8px;font-size:11px;cursor:pointer;flex-shrink:0">✕</button>
      </div>`).join('');
  }

  // Auto-import existing points saved by this user before cw_my_pts tracking existed
  function importExistingMyPts(){
    const nick=(localStorage.getItem('cw_nickname')||'').toLowerCase().trim();
    const identity=getIdent();
    const name=(identity?.name||nick||'').toLowerCase().trim();
    const existingIds=new Set(getMyPts().map(p=>p.id));

    const candidates=[];

    // 1 — cw_local_pts: ALL locally saved points (id starts 'l_') — always belong to this device/user
    try{
      const arr=JSON.parse(localStorage.getItem('cw_local_pts')||'[]');
      arr.forEach(p=>{if(p.lat&&p.lng&&!existingIds.has(p.id))candidates.push(p);});
    }catch(e){}

    // 2 — S.commPts in memory: local items (id starts 'l_' or 'local_') also belong to this user
    (window.S?.commPts||[]).forEach(p=>{
      if(!existingIds.has(p.id)&&p.lat&&p.lng){
        const isLocal=String(p.id||'').startsWith('l_')||String(p.id||'').startsWith('local_');
        if(isLocal){
          candidates.push(p);
        } else if(name){
          // Supabase points: match by contributor name
          const cn=(p.contributor_name||'').toLowerCase().trim();
          if(cn&&(cn===name||cn.includes(name)||name.includes(cn)))candidates.push(p);
        }
      }
    });

    candidates.forEach(p=>addMyPt({
      id:p.id||'mine_'+Date.now(),
      lat:p.lat,lng:p.lng,
      water_type:p.water_type||'drinking_water',
      notes:p.notes||'',
      source:'mine',
      saved_at:p.created_at||p.saved_at||new Date().toISOString()
    }));
  }

  function injectMinePtsLayer(){
    const map=getMap();if(!map||map.getSource('wmine'))return;
    map.addSource('wmine',{type:'geojson',data:{type:'FeatureCollection',features:[]}});
    map.addLayer({id:'wmine-l',type:'circle',source:'wmine',paint:{
  ...
  // Patch point submission (Panel tab "Add Water Point" form)
  function patchSubmitPt(){
    const orig=window.submitPt;if(!orig)return;
    window.submitPt=function(){
      // Capture coords from S.addPt before form resets
      const ap=window.S?.addPt;
      const type=document.querySelector('#addpt-bod .tb.active')?.dataset?.type||
                 document.querySelector('#addpt-bod .tb.active')?.textContent?.toLowerCase().trim()||'fountain';
      const notes=document.getElementById('pt-notes')?.value||'';
      orig();
      if(ap?.lat&&ap?.lng){
        addMyPt({id:'mine_'+Date.now(),lat:ap.lat,lng:ap.lng,
          water_type:type,notes,source:'mine',saved_at:new Date().toISOString()});
      }
    };
  }

  // Patch Water tab save
  function patchWaterTabSave(){
    const orig=window.cwWaterTabSave;if(!orig)return;
    window.cwWaterTabSave=function(){
      const f=document.getElementById('wt-add-form');
      const lat=f?._lat,lng=f?._lng;
      const type=document.getElementById('wt-type')?.value||'drinking_water';
      const notes=document.getElementById('wt-note')?.value||'';
      orig();
      if(lat!=null&&lng!=null){
        addMyPt({id:'mine_'+Date.now(),lat,lng,
          water_type:type,notes,source:'mine',saved_at:new Date().toISOString()});
      }
    };
  }
...
<div class="card" style="background:linear-gradient(135deg,rgba(14,165,233,.1),rgba(56,189,248,.05));border-color:rgba(14,165,233,.2)">
  ...
  <button onclick="openLB&&openLB()" style="width:100%;padding:11px;border:none;border-radius:12px;background:linear-gradient(135deg,#F59E0B,#EAB308);color:#1C1917;font-size:14px;font-weight:700;cursor:pointer;margin-bottom:12px">🏆 Leaderboard</button>
</div>
<div class="card" style="margin-top:10px">
  <div class="stog col" onclick="toggleSec('my-pts-bod',this);window.updateMyPtsList&&updateMyPtsList()">
    <span class="ct" style="margin:0">📍 My Water Points</span>
    <div style="display:flex;align-items:center;gap:8px;margin-left:auto">
      <span id="my-pts-count" style="background:rgba(245,158,11,.15);color:#F59E0B;border-radius:10px;padding:2px 8px;font-size:11px;font-weight:700"></span>
      <span class="arr">▼</span>
    </div>
  </div>
  <div class="sbod hid" id="my-pts-bod">
    <p style="font-size:10px;color:#7B91B0;margin:0 0 8px">These are YOUR added water points. Visible on the map when you open this tab.</p>
    <div id="my-pts-list"></div>
  </div>
</div>`;
    sc.appendChild(div);
    ...
    updateProfileUI();
    const lb=document.getElementById('lb-card');if(lb)lb.style.display=window._cwSb?'':'none';
    updateMyPtsList();
    refreshMyPtsLayer();
  }

  window.updateMyPtsList=updateMyPtsList;
  ...
  function showPanelContent(){
    ...
    // Restore clear route button if a route is loaded
    const cBtn=document.getElementById('cw-clear-btn');
    if(cBtn) cBtn.style.display=hasRoute?'':'none';
  }
  function showProfileContent(){
    ...
    // Hide clear route button — not relevant on profile tab
    const cBtn=document.getElementById('cw-clear-btn');
    if(cBtn) cBtn.style.display='none';
    // Auto-import any existing community points that belong to this user
    importExistingMyPts();
    updateProfileUI();
    updateMyPtsList();
    refreshMyPtsLayer();
  ...
  function setSheetState(sb,state){
    // Only update panel height — NEVER change active tab, content, or map layers
    // Tab switches are handled exclusively by the nav buttons
  ...
  function initSwipe(){
    ...
    hdr.addEventListener('touchend',e=>{
      ...
      // swipe down — Profile minimum is 'half'; others minimum 'peek'
      const minIdx=_cwTab==='profile'?2:1;
      if(dy>0&&idx>minIdx) setSheetState(sb,STATES[idx-1]);
  ...
  function patchUpdateList(){
    ...
    window.updateList=function(){
      ...
      if(!S)return;
      const hidden=new Set((S.hiddenWptIds||[]).map(String));
      const act=(S.waterPts||[]).filter(w=>!REPORTED.has(w.id)&&!hidden.has(String(w.id))).map(p=>({...p,_s:'osm'}));
      const rep=(S.waterPts||[]).filter(w=>REPORTED.has(w.id)&&!hidden.has(String(w.id))).map(p=>({...p,_s:'reported'}));
      const all=[...act,...(S.commPts||[]).filter(p=>!hidden.has(String(p.id))).map(p=>({...p,_s:'community'})),...rep];
  ...
  // ─── MAP DENSITY FILTER ───────────────────────────────────────
  function waterPointScore(w){
    const type=String(w?.water_type||'').toLowerCase();
    const base={drinking_water:5,fountain:4,water_tap:4,tap:4,spring:4,picnic_site:2,toilets:1,shop:1,bar:1,grave_yard:0,cemetery:0}[type]??2;
    return base+(w?._cwKind==='community'?1:0);
  }

  // Enforce a real minimum route distance. Bucket boundaries could otherwise
  // leave two points only a few metres apart when they fell into neighbouring buckets.
  function sparseRoutePoints(points,rpts,interval){
    if(!interval||interval<=0||!Array.isArray(rpts)||rpts.length<2)return points||[];
    const items=(points||[]).map(w=>({...w,_cwRouteKm:window.nearestOnRoute(w,rpts).km}))
      .sort((a,b)=>a._cwRouteKm-b._cwRouteKm);
    const out=[];let i=0;
    while(i<items.length){
      const seedKm=items[i]._cwRouteKm;
      let j=i;
      while(j<items.length&&items[j]._cwRouteKm<seedKm+interval)j++;
      const target=seedKm+interval/2;
      const bucket=items.slice(i,j).sort((a,b)=>{
        const byCenter=Math.abs(a._cwRouteKm-target)-Math.abs(b._cwRouteKm-target);
        return byCenter||waterPointScore(b)-waterPointScore(a);
      });
      const chosen=bucket[0];
      if(!chosen)break;
      out.push(chosen);
      const nextKm=chosen._cwRouteKm+interval;
      i=items.findIndex((item,idx)=>idx>=j&&item._cwRouteKm>=nextKm);
      if(i<0)break;
    }
    return out.map(({_cwRouteKm,...w})=>w);
  }
  window.cwSparseRoutePoints=sparseRoutePoints;
  ...
  function injectDensitySetting(){
    ...
    div.innerHTML=`<div><div class="sl">Map point spacing</div><div class="ss">0 = show all; otherwise minimum distance</div></div><div class="si"><input type="number" id="map-int" value="0" min="0" max="50" step="1" oninput="window.refreshLayers&&refreshLayers()"><span class="unit">km</span></div>`;
    gpxRow.parentNode.insertBefore(div,gpxRow);
  }

  function applyTabLayers(){
    const map=getMap(),S=window.S;if(!map)return;
    const panelActive=_cwTab==='panel'||document.getElementById('bnav-panel')?.classList.contains('vactive');
    const v=panelActive?'panel':_cwTab;
    const EMPTY={type:'FeatureCollection',features:[]};
    ['wosm-l','wrep-l','wcomm-l'].forEach(id=>{try{map.setLayoutProperty(id,'visibility',v==='panel'?'visible':'none');}catch(e){}});
    ...
    if(v==='profile'){
      // Clean map — only show the user's own added water points
      ['route-l','route-h','route-arrows-l'].forEach(id=>{try{map.setLayoutProperty(id,'visibility','none');}catch(e){}});
      ['wosm','wrep','wcomm','wfw'].forEach(s=>map.getSource(s)?.setData(EMPTY));
      refreshMyPtsLayer(); // show wmine layer (personal points)
      return;
    }

    ['route-l','route-h','route-arrows-l'].forEach(id=>{try{map.setLayoutProperty(id,'visibility',v==='water'?'none':'visible');}catch(e){}});
    // Hide personal layer on non-profile tabs
    try{map.getSource('wmine')?.setData(EMPTY);}catch(e){}
    ...
    if(v==='water'){
      ...
      // Panel tab — filter community points to only show near current route
      map.getSource('wfw')?.setData(EMPTY);
      if(S){
        const interval=parseFloat(document.getElementById('map-int')?.value)||0;
        const act=(S.waterPts||[]).filter(w=>!REPORTED.has(w.id)&&!window.cwIsHidden?.(w)).map(w=>({...w,_cwKind:'osm'}));
        const rep=(S.waterPts||[]).filter(w=>REPORTED.has(w.id)&&!window.cwIsHidden?.(w));
        // Only show community points near the loaded route (not globally)
        const rpts=S.routePts||[];
        const nearComm=rpts.length
          ? (S.commPts||[]).filter(w=>!window.cwIsHidden?.(w)&&ptNearRoute(w,rpts,1000)).map(w=>({...w,_cwKind:'community'}))
          : [];
        const sparse=sparseRoutePoints([...act,...nearComm],rpts,interval);
        map.getSource('wosm')?.setData(fc(sparse.filter(w=>w._cwKind==='osm')));
        map.getSource('wrep')?.setData(fc(rep));
        map.getSource('wcomm')?.setData(fc(sparse.filter(w=>w._cwKind==='community')));
      }
    }
  }
  ...
  function patchRefreshLayers(){
    ...
    window.refreshLayers=function(){
      ...
      const REPORTED=window.REPORTED||new Set();
      const act=S.waterPts.filter(w=>!REPORTED.has(w.id)&&!window.cwIsHidden?.(w)).map(w=>({...w,_cwKind:'osm'}));
      const comm=S.commPts.filter(w=>!window.cwIsHidden?.(w)).map(w=>({...w,_cwKind:'community'}));
      const sparse=sparseRoutePoints([...act,...comm],S.routePts||[],interval);
      const rep=(S.waterPts||[]).filter(w=>REPORTED.has(w.id)&&!window.cwIsHidden?.(w));
      const fc=arr=>({type:'FeatureCollection',features:(arr||[]).map(w=>({type:'Feature',geometry:{type:'Point',coordinates:[w.lng,w.lat]},properties:{...w}}))});
      map.getSource('wosm')?.setData(fc(sparse.filter(w=>w._cwKind==='osm')));
      map.getSource('wrep')?.setData(fc(rep));
  ...
  window.cwWaterTabAdd=function(){
    ...
    if(!form){
      ...
      form.style.cssText='position:fixed;bottom:calc(78px + env(safe-area-inset-bottom));left:12px;right:12px;z-index:2000;background:rgba(13,20,38,.96);border:1px solid rgba(255,255,255,.12);border-radius:18px;padding:16px;backdrop-filter:blur(14px);box-shadow:0 8px 32px rgba(0,0,0,.5)';
...
<div style="display:flex;gap:8px">
  <button onclick="if(window.S)window.S._waterTabAdding=false;document.getElementById('wt-add-form').remove()" style="flex:1;padding:10px;border:1px solid rgba(255,255,255,.1);border-radius:10px;background:none;color:#7B91B0;font-size:12px;cursor:pointer">Cancel</button>
  ...
  window.cwWaterTabTapMap=function(){
    const f=document.getElementById('wt-add-form');
    if(window.S)window.S._waterTabAdding=true;
    const map=getMap();if(!map)return;
    map.getCanvas().style.cursor='crosshair';
    const handler=e=>{
      map.getCanvas().style.cursor='';map.off('click',handler);
      if(window.S)window.S._waterTabAdding=false;
  ...
  window.cwWaterTabSave=async function(){
    const f=document.getElementById('wt-add-form');if(!f||f._lat==null)return;
    const type=document.getElementById('wt-type')?.value||'drinking_water';
    const note=document.getElementById('wt-note')?.value||'';
    const ok=await window.saveCommPt?.({lat:f._lat,lng:f._lng,water_type:type,notes:note,contributor_name:window.getNickname?.()||'Anonymous',points:10});
    if(ok===false)return;
    const tot=parseInt(localStorage.getItem('cw_total_wpts')||'0')+1;localStorage.setItem('cw_total_wpts',String(tot));
    f.remove();if(window.S)window.S._waterTabAdding=false;window.cwWQAddedWater?.();window.refreshLayers?.();window.showToast?.('✅ Water point saved');
  };
  ...
  function moveCommunityCardsToProfile(){
    const prof=document.getElementById('profile-section');if(!prof)return;
    const db=document.getElementById('sb-bod')?.closest('.card');
    const lb=document.getElementById('lb-card');
    [db,lb].forEach(card=>{if(card&&card.parentElement!==prof)prof.appendChild(card);});
  }
  ...
  // Faster, bounded water query. It keeps the original route lifecycle but
  // avoids the very large cemetery/way/relation query that can hang Overpass.
  function patchStableQOverpass(){
    const base=window.qOverpass;if(!base||window._cwStableQ)return;window._cwStableQ=true;
    window.qOverpass=async function(rpts,buf){
      const sp=samplePts(rpts,40),cs=sp.map(p=>`${p.lat},${p.lng}`).join(','),ar=`(around:${buf},${cs})`;
      const q=`[out:json][timeout:12];(node["amenity"="drinking_water"]${ar};node["man_made"="water_tap"]["drinking_water"!="no"]${ar};node["natural"="spring"]${ar};node["amenity"="fountain"]${ar};node["tourism"="picnic_site"]["drinking_water"="yes"]${ar};node["amenity"="toilets"]["drinking_water"="yes"]${ar};);out tags;`;
      for(const ep of OVR){
        const ac=new AbortController(),tm=setTimeout(()=>ac.abort(),12000);
        try{
          const res=await fetch(ep,{method:'POST',body:new URLSearchParams({data:q}),headers:{'Content-Type':'application/x-www-form-urlencoded'},signal:ac.signal});
          if(!res.ok)continue;const d=await res.json();const seen=new Set();
          return (Array.isArray(d.elements)?d.elements:[]).filter(e=>e.type==='node'&&!seen.has(e.id)&&seen.add(e.id)).map(e=>({id:`osm_${e.id}`,lat:e.lat,lng:e.lon,water_type:e.tags?.amenity||e.tags?.man_made||e.tags?.natural||'drinking_water',name:e.tags?.name||null,seasonal:e.tags?.seasonal==='yes',source:'osm'}));
        }catch(e){
          if(e.name!=='AbortError')continue;
        }finally{clearTimeout(tm);}
      }
      // Last resort, use the original query once, but cap its wait time.
      return Promise.race([base(rpts,buf),new Promise((_,rej)=>setTimeout(()=>rej(new Error('Overpass timeout')),15000))]);
    };
  }
  ...
  function init(){
    ...
    patchResetRoute();
    patchRouteBanner();
    patchStableQOverpass();
    patchUpdateStats();
    patchSubmitPt();
    // Water-tab save uses the shared saveCommPt path directly.
    initPreGeocode();
    patchInitSB();
    injectLeaderboard();
    moveCommunityCardsToProfile();
    autoConnectSB();
    // Expose helpers for cw-fixes.js
    window._cwGetTab=()=>_cwTab;
    window._cwProfileRefresh=()=>{if(_cwTab==='profile'){showProfileContent();applyTabLayers();}};
    // Start on Panel tab with half-sheet
    if(window.innerWidth<=768) setTimeout(()=>window.setView?.('panel'),300);
    // Set initial layer state + inject personal water layer once map is loaded
    const _mapReady=()=>{injectMinePtsLayer();applyTabLayers();};
