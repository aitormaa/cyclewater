...
(function(){
  ...
  // Patch finishLoad to inject arrows after route loads
  (function(){
    const _orig=window.finishLoad;
    window.finishLoad=function(pts){
      _orig?.(pts);
      const map=window.map||window.cwMap;if(!map||!pts?.length)return;
      try{
        setupArrowLayer(map);
        map.getSource('route-arrows')?.setData(computeArrowFeatures(pts));
      }catch(e){}
    };
  })();
  ...
  (function(){
    ...
    if(_orig){window.resetRoute=window.clearRoute=function(){_orig();clearArrows();};}
  })();
