...
<div class="card" style="margin-top:10px">
  ...
  function ensureWaterMarkerLayer(map){
    if(!map||!map.getSource('wosm')||map.getLayer('wosm-force-l'))return;
    try{map.addLayer({id:'wosm-force-l',type:'circle',source:'wosm',paint:{'circle-radius':9,'circle-color':'#0EA5E9','circle-stroke-width':3,'circle-stroke-color':'#FFFFFF','circle-opacity':.98}});}catch(e){}
  }

  function applyTabLayers(){
    const map=getMap(),S=window.S;if(!map)return;
    ensureWaterMarkerLayer(map);
    const v=(_cwTab==='panel'||document.getElementById('bnav-panel')?.classList.contains('vactive'))?'panel':_cwTab;
    ...
    if(v==='profile'){
      ...
      ['wosm-l','wrep-l','wcomm-l','wfw-l'].forEach(id=>{try{map.setLayoutProperty(id,'visibility','none');}catch(e){}});
    ...
    ['wosm-l','wrep-l','wcomm-l','wosm-force-l'].forEach(id=>{try{map.setLayoutProperty(id,'visibility',v==='panel'?'visible':'none');}catch(e){}});
    try{map.setLayoutProperty('wfw-l','visibility',v==='water'?'visible':'none');}catch(e){}
