...
(function(){
  ...
  // ─── PATCH qOverpass (route loading) — parallel + 12h cache ──
  function patchQOverpass(){
    ...
    window.qOverpass=async function(rpts,buf){
      ...
      const sp=window.samplePts?samplePts(rpts,80):rpts.filter((_,i,a)=>i===0||i===a.length-1||i%Math.ceil(a.length/80)===0);
      const cs=sp.map(p=>`${p.lat},${p.lng}`).join(','),ar=`(around:${buf},${cs})`;
      const q=`[out:json][timeout:60];(node["amenity"="drinking_water"]${ar};node["man_made"="water_tap"]["drinking_water"!="no"]${ar};node["natural"="spring"]["drinking_water"="yes"]${ar};node["amenity"="fountain"]["drinking_water"="yes"]${ar};node["tourism"="picnic_site"]["drinking_water"="yes"]${ar};node["amenity"="toilets"]["drinking_water"="yes"]${ar};node["amenity"="grave_yard"]${ar};way["amenity"="grave_yard"]${ar};relation["amenity"="grave_yard"]${ar};node["landuse"="cemetery"]${ar};way["landuse"="cemetery"]${ar};relation["landuse"="cemetery"]${ar};);out center;`;
      let d;try{d=await fastOverpass(q);}catch(e){throw new Error('Overpass unreachable');}
      const seen=new Set();
      const pts=d.elements.filter(e=>{if(!['node','way','relation'].includes(e.type)||seen.has(e.id))return false;seen.add(e.id);return true;}).map(e=>{const lat=e.center?e.center.lat:e.lat,lng=e.center?e.center.lon:e.lon;return{id:`osm_${e.id}`,lat,lng,water_type:e.tags?.amenity||e.tags?.man_made||e.tags?.natural||e.tags?.landuse||'drinking_water',name:e.tags?.name||null,seasonal:e.tags?.seasonal==='yes',source:'osm'};});
...
<div class="card" style="background:linear-gradient(135deg,rgba(14,165,233,.1),rgba(56,189,248,.05));border-color:rgba(14,165,233,.2)">
  ...
  function enhanceSetView(){
    ...
    window.setView=function(v){
      if(v==='split')v='panel'; // backwards compat with finishLoad
      _base(v);
      applyTabLayers();
