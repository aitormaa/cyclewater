...
(function(){
  ...
  window.cwSaveRoute=function(name,pts,km,gpx){
    ...
    filtered.unshift({name,km:Math.round(km||0),date:new Date().toLocaleDateString('en-GB',{day:'2-digit',month:'short'}),gpxKey});
        ...
        ${routes.map((r,i)=>`
          <div style="display:flex;align-items:center;gap:10px;padding:10px 12px;background:var(--c1);border:1px solid var(--br);border-radius:12px;margin-bottom:7px;cursor:pointer" onclick="cwReloadRoute(${i})">
            ...
            <div style="flex:1;min-width:0">
              ...
              <div style="font-size:10px;color:var(--tm);margin-top:2px">${r.km?r.km+'km · ':''}${r.date}${r.gpxKey?' · ⚡ instant':' · 📁 reupload'}</div>
            </div>
