...
<div class="card" style="background:linear-gradient(135deg,rgba(14,165,233,.1),rgba(56,189,248,.05));border-color:rgba(14,165,233,.2)">
  ...
  function patchResetRoute(){
    ...
    window.resetRoute=function(e){
      orig(e);
      // Restore FindWater layer visibility
      setLayerVis(FW_LAYERS,'visible');
    };
  }
  ...
  function applyTabLayers(){
    ...
    if(v==='water'){
      setLayerVis(TRACK_LAYERS,'none');setLayerVis(FW_LAYERS,'visible');
    } else {
      setLayerVis(TRACK_LAYERS,'visible');setLayerVis(FW_LAYERS,'none');
    }
  }
  ...
  window.cwFindWater=async function(){
    ...
    const loc=(locEl?.value||'').trim(),r=parseInt(rEl?.value)||2;
    ...
    for(const ep of OVR){
      try{
        const resp=await fetch(ep,{method:'POST',body:new URLSearchParams({data:q})});
...
<div style="display:flex;gap:8px">
  ...
  function init(){
    ...
    enhanceSetView();
    initSwipe();
    setTimeout(injectDensitySetting,300);
    setTimeout(patchRefreshLayers,400);
    setTimeout(patchUpdateList,500);
    setTimeout(patchFinishLoad,200);
    setTimeout(patchResetRoute,200);
    patchInitSB();
    autoConnectSB();
