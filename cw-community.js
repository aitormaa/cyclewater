...
// ── Water point gap distances — click on route between two water points ──
(function(){
  const R=6371;
  ...
  function mkCum(pts){const d=[0];for(let i=1;i<pts.length;i++)d.push(d[i-1]+hv(pts[i-1],pts[i]));return d;}
  function nearKm(pt,pts,d){let mi=0,md=9e9;for(let i=0;i<pts.length;i++){const x=hv(pt,pts[i]);if(x<md){md=x;mi=i;}}return d[mi];}
  let done=0;
  function setupGapClick(){
    if(done)return;done=1;
    const map=window.map||window.cwMap;if(!map)return;
    map.on('click','route-l',function(e){
      const pts=window.S?.routePts;
      const wpts=[...(window.S?.waterPts||[]),...(window.S?.commPts||[])].filter(p=>p.lat&&p.lng);
      if(!pts?.length||!wpts.length)return;
      const cd=mkCum(pts);
      const ck=nearKm({lat:e.lngLat.lat,lng:e.lngLat.lng},pts,cd);
      const sw=wpts.map(p=>({...p,km:nearKm(p,pts,cd)})).sort((a,b)=>a.km-b.km);
      const pv=sw.filter(p=>p.km<=ck).pop();
      const nx=sw.find(p=>p.km>ck);
      if(!pv&&!nx)return;
      let html;
      if(pv&&nx) html=`💧→💧 <b>${(nx.km-pv.km).toFixed(1)} km</b> between water points`;
      else if(nx)  html=`Next 💧 in <b>${(nx.km-ck).toFixed(1)} km</b>`;
      else         html=`Last 💧 was <b>${(ck-pv.km).toFixed(1)} km</b> ago`;
    ...
    map.on('mouseenter','route-l',()=>{map.getCanvas().style.cursor='crosshair';});
    map.on('mouseleave','route-l',()=>{map.getCanvas().style.cursor='';});
  }
  // Hook into finishLoad so it runs after every route load
  const _fi=window.finishLoad;
  window.finishLoad=function(p){_fi?.(p);setTimeout(setupGapClick,600);};
  // Also try immediately if map already ready
  setTimeout(setupGapClick,2000);
})();
