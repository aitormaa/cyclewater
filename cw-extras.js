...
(function(){
  ...
  // Auto-import existing community points saved by this user (saved before cw_my_pts tracking existed)
  function importExistingMyPts(){
    ...
    const existing=getMyPts();
    const existingIds=new Set(existing.map(p=>p.id));

    const candidates=[];
    // 1 — from S.commPts loaded from Supabase
    (window.S?.commPts||[]).forEach(p=>{
      if(!existingIds.has(p.id)&&p.lat&&p.lng){
        const cn=(p.contributor_name||'').toLowerCase().trim();
        if(name&&(cn===name||cn.includes(name)||name.includes(cn)))candidates.push(p);
      }
    });
    // 2 — from localStorage keys used by cw-community.js
    ['cw_comm_pts','cw_pts','local_comm_pts'].forEach(key=>{
      try{
        const arr=JSON.parse(localStorage.getItem(key)||'[]');
        arr.forEach(p=>{
          if(!existingIds.has(p.id)&&p.lat&&p.lng){
            const cn=(p.contributor_name||'').toLowerCase().trim();
            if(!name||cn===name||cn.includes(name)||name.includes(cn)){
              candidates.push(p);
            }
          }
        });
