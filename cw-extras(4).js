...
<div class="card" style="background:linear-gradient(135deg,rgba(14,165,233,.1),rgba(56,189,248,.05));border-color:rgba(14,165,233,.2)">
  ...
  function initSwipe(){
    ...
    hdr.addEventListener('touchend',e=>{
      ...
      if(dy>0&&idx>0) setSheetState(sb,STATES[idx-1]); // swipe down → smaller
      else if(dy<0&&idx<STATES.length-1) setSheetState(sb,STATES[idx+1]); // swipe up → bigger
    },{passive:true});
  }
  ...
  window.cwFindWater=async function(){
    ...
    if(map){
      ...
      map.flyTo({center:[lng,lat],zoom:r<=1?14:r<=3?12:r<=10?11:10,speed:1.4});
    }
    if(res){
