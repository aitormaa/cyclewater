...
(function () {
  ...
  style.textContent = `
    ...
    .cw-sr-item:hover,.cw-sr-item.sel{background:#334155}
    ...
    @media(max-width:768px){
      #cw-search-mobile-wrap{position:relative;flex-shrink:0}
      #cw-search{position:static;width:100%;max-width:100%}
      #cw-search-box{border-radius:0;border:none;border-bottom:1px solid #334155;padding:10px 14px;box-shadow:none;background:#1E293B}
      #cw-search-results{position:absolute;left:0;right:0;z-index:20;border-radius:0;margin-top:0;border-left:none;border-right:none;border-top:none}
    }
  `;
  ...
  function injectUI() {
    ...
    document.addEventListener('click', e => {
      const wrap = document.getElementById('cw-search-mobile-wrap') || document.getElementById('cw-search');
  ...
  function onInput(e) {
    ...
    debounceTimer = setTimeout(() => search(q, false), 400);
  }
  ...
  function onKeydown(e) {
    ...
    if (e.key === 'ArrowDown') {
      ...
      e.preventDefault();
      const q = document.getElementById('cw-search-input').value.trim();
      if (!q) return;
      // If we already have results, pick one
      const pick = selectedIdx >= 0 ? results[selectedIdx] : results[0];
      if (pick) {
        selectResult(pick);
      } else {
        // No results yet — fire immediate search then auto-select first result
        clearTimeout(debounceTimer);
        search(q, true); // autoSelect = true
      }
  ...
  async function search(q, autoSelect) {
    if (searching) return;
    searching = true;
    ...
    try {
      ...
      if (autoSelect && results[0]) selectResult(results[0]);
    } catch (e) {
  ...
  function renderResults(data) {
    ...
    el.innerHTML = data.map((r, i) => {
      ...
      return `<div class="cw-sr-item" onclick="cwSearchSelect(${i})">
        <div class="cw-sr-name">${icon} ${parts[0].trim()}</div>
        ${parts[1] ? `<div class="cw-sr-sub">${parts.slice(1,3).map(s=>s.trim()).join(', ')}</div>` : ''}
      </div>`;
    }).join('');
    el.style.display = 'block';
  }
  ...
  function selectResult(r) {
    ...
    if (map) {
      if (r.boundingbox) {
        ...
        const zoom = t==='country'?5:t==='state'?7:t==='county'?9:t==='city'?11:t==='town'||t==='suburb'?13:14;
  ...
  function init() {
    if (document.getElementById('view-bar') || document.getElementById('map-container')) injectUI();
