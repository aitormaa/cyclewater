...
(function(){
  ...
  // ─── CLUSTER LAYERS + OWN ROUTE LAYERS ON TOP (#5) ───────────
  // We add our OWN route layers (cw-route-bg, cw-route-ln, cw-route-ar)
  // on top of everything instead of fighting with moveLayer on route-l/route-h.
  // These use the same 'route' source so they update automatically when GPX loads.
  function setupClusters(){
    ...
    // ── 2. Direction arrows — added after route-l so they're on top ────
    if(!map.getLayer('cw-route-ar')){
      try{
        // Draw a chevron arrow on a canvas
        const sz=32,cv=document.createElement('canvas');cv.width=sz;cv.height=sz;
        const cx=cv.getContext('2d');
        cx.clearRect(0,0,sz,sz);
        cx.strokeStyle='rgba(255,255,255,0.95)';cx.lineWidth=3.5;
        cx.lineCap='round';cx.lineJoin='round';
        cx.beginPath();cx.moveTo(9,10);cx.lineTo(22,16);cx.lineTo(9,22);cx.stroke();
        const raw=new Uint8Array(cx.getImageData(0,0,sz,sz).data.buffer);
        if(!map.hasImage('cw-arr'))map.addImage('cw-arr',{width:sz,height:sz,data:raw},{sdf:false});
        map.addLayer({id:'cw-route-ar',type:'symbol',source:'route',
          layout:{'symbol-placement':'line','symbol-spacing':100,
            'icon-image':'cw-arr','icon-size':0.75,
            'icon-allow-overlap':true,'icon-keep-upright':false,
            'icon-rotation-alignment':'map'}});
      }catch(e){}
    }
  }

  // Show/hide route layers — route-h/route-l are native; cw-route-ar is our arrow layer
  window._cwSetRouteVis=function(vis){
    const map=window.map||window.cwMap;if(!map)return;
    ['route-h','route-l','cw-route-ar'].forEach(id=>{
  ...
  function onMapReady(){
    ...
    // ── Guaranteed route visibility: fires every time route source data changes ──
    // This is bulletproof — regardless of layer order or applyTabLayers timing,
    // whenever loadRouteData sets route source data we force route-h/route-l visible.
    const map=window.map||window.cwMap;
    if(map){
      map.on('sourcedata',function(e){
        if(e.sourceId==='route'&&e.isSourceLoaded){
          ...
          if(tab!=='profile'&&tab!=='water'){
            setTimeout(()=>{
              try{map.setLayoutProperty('route-h','visibility','visible');}catch(_){}
              try{map.setLayoutProperty('route-l','visibility','visible');}catch(_){}
              try{map.setLayoutProperty('cw-route-ar','visibility','visible');}catch(_){}
            },80);
          }
        }
      });
  ...
  // ─── Arrow layer visibility — follow route-l but not managed by cw-extras ─
  // cw-extras' applyTabLayers manages route-h/route-l but not cw-route-ar.
  // This minimal setView hook ONLY controls cw-route-ar visibility.
  (function(){
    const _sv=window.setView;
    window.setView=function(v){
      _sv?.(v);
      const m=window.map||window.cwMap;if(!m)return;
      const vis=(v==='water'||v==='profile')?'none':'visible';
      setTimeout(()=>{try{m.setLayoutProperty('cw-route-ar','visibility',vis);}catch(_){}},60);
    };
  })();
  ...
  // ─── 4. SAFETY NET — ensure cw-route layers are visible after data refresh ─
  const _origRL2=window.refreshLayers;
  window.refreshLayers=function(){
    _origRL2?.();
    const tab=window._cwGetTab?.()||'panel';
    const vis=(tab!=='profile'&&tab!=='water')?'visible':'none';
    window._cwSetRouteVis?.(vis);
  };
