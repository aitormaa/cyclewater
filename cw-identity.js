...
(function () {
  ...
  function openModal(prefill) {
    ...
    const existingId = getIdentity() || {};
    const isUpdate = !!(existingId.setup);

    const modal = document.createElement('div');
    modal.id = 'cw-id-modal';
    modal.innerHTML = `
      <div id="cw-id-modal-box">
        <h2>👤 ${isUpdate ? 'Your Profile' : 'Welcome to CycleWater'}</h2>
        <p id="cw-id-msg">${isUpdate ? `Logged in as <strong>${existingId.name||'cyclist'}</strong>. Update your profile below.` : 'Set your name once — it will appear on every water point you add.'}</p>
        <label>Display name *</label>
        <input id="cw-id-name-inp" type="text" placeholder="e.g. Aitor" maxlength="30" value="${id.name||''}">
        <label>Email (optional — used to reopen your account)</label>
        ...
        <div class="row">
          ...
          <button class="btn-p" id="cw-id-save-btn" onclick="cwIdentitySave()">${isUpdate ? 'Update' : 'Create account'}</button>
        </div>
      </div>`;
    document.body.appendChild(modal);

    // Email recognition: when user enters a known email, auto-restore profile
    const emailInp = document.getElementById('cw-id-email-inp');
    const nameInp  = document.getElementById('cw-id-name-inp');
    const msgEl    = document.getElementById('cw-id-msg');
    const saveBtn  = document.getElementById('cw-id-save-btn');
    emailInp.addEventListener('blur', async () => {
      const entered = emailInp.value.trim().toLowerCase();
      if (!entered) return;

      // 1. Same-device check (localStorage)
      const stored = getIdentity();
      if (stored && stored.email && stored.email.toLowerCase() === entered) {
        nameInp.value = stored.name || nameInp.value;
        msgEl.innerHTML = `✅ Welcome back, <strong>${stored.name||'cyclist'}</strong>! Your profile has been restored.`;
        saveBtn.textContent = 'Sign in';
        return;
      }

      // 2. Cross-device check via Supabase
      const sb = window._cwSb;
      if (!sb) return;
      try {
        msgEl.innerHTML = `<span style="color:#64748B">🔍 Looking up your account…</span>`;
        const { data, error } = await sb
          .from('community_water_points')
          .select('contributor_name, contributor_email')
          .eq('contributor_email', entered)
          .limit(1);
        if (!error && data && data.length > 0) {
          const dbName = data[0].contributor_name || '';
          // Restore identity from DB
          saveIdentity({ name: dbName, email: entered, setup: true });
          localStorage.setItem('cw_nickname', dbName);
          nameInp.value = dbName;
          msgEl.innerHTML = `✅ Account found! Welcome back, <strong>${dbName||'cyclist'}</strong>. Your water points will be restored.`;
          saveBtn.textContent = 'Sign in';
        } else {
          msgEl.innerHTML = ``;  // new user, no match
        }
      } catch(e) {
        msgEl.innerHTML = ``;
      }
