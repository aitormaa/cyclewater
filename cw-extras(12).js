...
<div class="card" style="background:linear-gradient(135deg,rgba(14,165,233,.1),rgba(56,189,248,.05));border-color:rgba(14,165,233,.2)">
  ...
  window.cwFindWater=async function(){
    ...
    if(map){
      ...
      map.flyTo({center:[lng,lat],zoom:r<=1?14:r<=3?12:r<=5?11:r<=15?10:9,speed:1.4});
    }
    if(res){
...
<div style="display:flex;gap:8px">
  ...
  window.cwLoadLeaderboard=async function(){
    ...
    document.getElementById('lb-card')?.style&&(document.getElementById('lb-card').style.display='');
    ...
    try{
      const{data,error}=await sb.from('cw_routes').select('*').order('created_at',{ascending:false}).limit(25);
      if(error)throw error;
      if(!data||!data.length){list.innerHTML='<div style="color:var(--tm);font-size:12px;text-align:center;padding:12px">No routes yet — upload one to be first! 🚴</div>';return;}
      const medals=['🥇','🥈','🥉'];
      list.innerHTML=data.map((r,i)=>`<div style="display:flex;align-items:center;gap:10px;padding:10px 12px;background:var(--c1);border:1px solid var(--br);border-radius:12px;margin-bottom:7px"><span style="font-size:18px;flex-shrink:0">${medals[i]||'🗺️'}</span><div style="flex:1;min-width:0"><div style="font-size:13px;font-weight:600;color:var(--tx);white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${r.name||'Unnamed'}</div><div style="font-size:10px;color:var(--tm);margin-top:2px">${r.km?r.km+'km · ':''}${r.water_count||0} 💧 · ${r.user_name||'Anonymous'}</div></div></div>`).join('');
    }catch(e){list.innerHTML=`<div style="color:var(--tm);font-size:11px;padding:8px">⚠️ Create table first:<br><code style="font-size:10px;word-break:break-all">create table cw_routes(id bigint generated always as identity primary key,name text,km int default 0,water_count int default 0,user_name text default 'Anonymous',created_at timestamptz default now());</code></div>`;}
  };

  function patchCwSaveRoute(){
    const orig=window.cwSaveRoute;if(!orig)return;
    window.cwSaveRoute=async function(name,pts,km,gpx){
      orig(name,pts,km,gpx);
      const sb=window._cwSb;if(!sb)return;
      const S=window.S,ident=getIdent();
      try{
        await sb.from('cw_routes').upsert({name,km:Math.round(km||0),water_count:S?.waterPts?.length||0,user_name:ident?.name||'Anonymous',created_at:new Date().toISOString()},{onConflict:'name'});
        window.cwLoadLeaderboard();
      }catch(e){}
    };
  }
