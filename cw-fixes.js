...
(function(){
  ...
  function setupClusters(){
    ...
    // ── 2. Our OWN route line layers — added LAST so they're always on top ──
    // Hide originals (they may have ordering issues) and use ours instead
    try{map.setLayoutProperty('route-h','visibility','none');}catch(e){}
    try{map.setLayoutProperty('route-l','visibility','none');}catch(e){}

    // White halo
    if(!map.getLayer('cw-route-bg')){
      try{
        map.addLayer({id:'cw-route-bg',type:'line',source:'route',
          layout:{'line-join':'round','line-cap':'round'},
          paint:{'line-color':'#ffffff','line-width':7,'line-opacity':0.55}});
      }catch(e){}
    }
    // Blue line on top of halo
    if(!map.getLayer('cw-route-ln')){
      try{
        map.addLayer({id:'cw-route-ln',type:'line',source:'route',
          layout:{'line-join':'round','line-cap':'round'},
          paint:{'line-color':'#38BDF8','line-width':4,'line-opacity':0.97}});
      }catch(e){}
    }
  ...
  // Show/hide our custom route layers (called from applyTabLayers & refreshLayers)
  window._cwSetRouteVis=function(vis){
    const map=window.map||window.cwMap;if(!map)return;
    ['cw-route-bg','cw-route-ln','cw-route-ar'].forEach(id=>{
  ...
  // ─── Route visibility bridge for tab switches ──────────────────
  // cw-extras' applyTabLayers doesn't know about our cw-route-* layers,
  // so we hook setView to manage their visibility ourselves.
  const _origSV=window.setView;
  window.setView=function(v){
    _origSV?.(v);
    const vis=(v!=='profile'&&v!=='water')?'visible':'none';
    // Slight delay so applyTabLayers in cw-extras runs first
    setTimeout(()=>window._cwSetRouteVis?.(vis),50);
  };
